...
function OrgGoalsContent({ params }: { params: Promise<{ orgId: string }> }) {
    ...
    const myMember = members.find(m => m.orgId === orgId && m.userId === user?.id);

    const visibleGoals = getVisibleGoals(goals, myMember);
    const filtered = filter === "all" ? visibleGoals : visibleGoals.filter((g: OrgGoal) => g.status === filter);

    const counts: Record<string, number> = {
        all: visibleGoals.length,
        in_progress: visibleGoals.filter((g: OrgGoal) => g.status === "in_progress").length,
        completed: visibleGoals.filter((g: OrgGoal) => g.status === "completed").length,
        blocked: visibleGoals.filter((g: OrgGoal) => g.status === "blocked").length,
        not_started: visibleGoals.filter((g: OrgGoal) => g.status === "not_started").length,
    };
