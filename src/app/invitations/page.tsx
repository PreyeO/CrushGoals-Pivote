...
export default function InvitationsPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start p-6 pt-16 bg-background relative overflow-hidden">
      {/* Ambient blobs */}
      <div className="absolute top-0 right-0 w-100 h-100 bg-primary/5 blur-[120px] rounded-full -mr-32 -mt-32 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-100 h-100 bg-[oklch(0.70_0.18_155)]/5 blur-[120px] rounded-full -ml-32 -mb-32 pointer-events-none" />
      ...
      <div className="max-w-xl w-full relative z-10">
        <header className="mb-8 animate-fade-in text-center">
          ...
          <p className="text-[13px] text-muted-foreground mt-1">
            ...
            {pendingInvites.length === 1 ? "invitation" : "invitations"} to
            review.
          </p>
        </header>
        ...
        {pendingInvites.length === 0 ? (
          ...
          <div className="space-y-3 animate-fade-in-up">
            {pendingInvites.map((invite) => (
              <div
                key={invite.id}
                className="group flex items-center justify-between p-5 rounded-2xl glass-card border-border/40 hover:border-amber-500/40 hover:bg-amber-500/3 transition-all duration-300"
              >
