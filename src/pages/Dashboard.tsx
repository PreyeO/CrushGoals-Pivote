...
const motivationalQuotes = [
  {
    quote: "Any effort beats zero. Always.",
    author: "Preye",
  },
  {
...
export default function Dashboard() {
  ...
  const [goalTaskCounts, setGoalTaskCounts] = useState<{
    [goalId: string]: { totalCompleted: number; totalCount: number };
  }>({});

  // Pull to refresh functionality
  const handleRefresh = async () => {
    // Refresh goals and tasks data
    await Promise.all([
      refreshGoals(),
      // Re-fetch task counts
      fetchTaskCounts(),
    ]);
  };

  const { isRefreshing, progress, isPulling } = usePullToRefresh({
  ...
  useEffect(() => {
    // Wait for user to be authenticated and goals to finish loading
    if (!user) return;
    if (goalsLoading) return;

    try {
      const hasSeenTour = localStorage.getItem(
        "crushgoals_onboarding_completed"
      );
      // Show tour if: user is authenticated, goals finished loading, user has no goals, and hasn't seen the tour
  ...
  }, [user, goalsLoading, goals.length]); // Changed from user?.id to user
  ...
  if (isLoading && !profile) {
    return (
      <div className="min-h-screen bg-background flex">
        ...
        <main
          className={cn(
            "flex-1 p-4 sm:p-6 lg:p-8 lg:pr-8 pt-16 lg:pt-8 transition-all duration-300",
            mainPaddingClass
          )}
        >
  ...
  const displayName =
    profile?.username || profile?.full_name?.split(" ")[0] || "Champion";
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <main
        className={cn(
          "flex-1 w-full max-w-full p-4 sm:p-6 lg:p-8 lg:pr-8 pt-16 lg:pt-8 transition-all duration-300",
          mainPaddingClass
        )}
      >
        {/* Trial Banner */}
        ...
        <div className="w-full max-w-full space-y-6">
          {/* 4 Stat Cards Grid - 1 column on mobile, 2 on sm+, 4 on lg+ */}
          <section
            className="w-full max-w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4 animate-slide-up opacity-0"
            style={{ animationDelay: "50ms" }}
          >
            {/* Today's Tasks */}
            <Card
              variant="glass"
              className="w-full max-w-full p-3 sm:p-4 hover-scale relative overflow-hidden"
            >
          ...
          {/* Today's Tasks */}
          <section
            className="mb-6 animate-slide-up opacity-0"
                ...
                {activeGoals.slice(0, 4).map((goal, index) => (
                  ...
                  >
                    <GoalCard
                      ...
                      totalTasksCompleted={
                        goalTaskCounts[goal.id]?.totalCompleted
                      }
            ...
            >
              ...
              <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-thin">
                {goals.length >= 1 && (
                  <Card
                    variant="glass"
                    className="p-2 sm:p-3 min-w-[100px] sm:min-w-[120px] flex-shrink-0 text-center hover-scale"
                  >
