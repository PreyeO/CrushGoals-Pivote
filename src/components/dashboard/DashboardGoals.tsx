...
export function DashboardGoals() {
    ...
    const myMemberIds = members.filter(m => m.userId === user?.id).map(m => m.id);

    // Filter for goals assigned to any of these member IDs
    const myGoals = goals.filter(g => g.assignedTo.some(id => myMemberIds.includes(id))).slice(0, 3);
    ...
    return (
        <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">My Active Goals</h2>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 stagger">
                {myGoals.map((goal) => (
                    <div key={goal.id} className="glass-card p-6 animate-fade-in-up group flex flex-col justify-between">
                        <div>
                            <div className="flex items-center justify-between mb-4">
                            ...
                            <h3 className="font-bold text-[14px] mb-1 leading-tight">{goal.title}</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex items-center justify-between text-[11px] mb-1.5">
                                    ...
                                    <span className="font-bold text-primary">{goal.currentValue}%</span>
                                </div>
