  ...
  return (
    ...
    >
      ...
      {leaderboard.map((entry, i) => (
        <div
          key={entry.memberId}
          className={`grid grid-cols-[50px_1fr_90px_110px_70px_90px] px-5 py-3.5 items-center hover:bg-accent/30 transition-colors ${entry.rank <= 3 ? "bg-primary/3" : ""} ${i < leaderboard.length - 1 ? "border-b border-border/15" : ""}`}
        >
          ...
          <div className="flex items-center gap-2 px-1">
            <Progress value={entry.completionRate} className="h-1.25 flex-1" />
