...
export function ReportTopBlockers({ topBlockers, totalBlocked }: ReportTopBlockersProps) {
    return (
                ...
                {topBlockers.length === 0 ? (
                    ...
                    topBlockers.map(([reason, count], i) => (
                        <div key={i} className="space-y-2">
                            ...
                            <Progress value={(count / totalBlocked) * 100} className="h-1.5 bg-destructive/10" />
                        </div>
                    ))
                )}
