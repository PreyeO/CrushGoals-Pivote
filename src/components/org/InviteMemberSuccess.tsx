...
export function InviteMemberSuccess({
    ...
    const isBatch = invitedEmails.length > 1 || inviteLink === "batch_success";
    ...
    return (
        <div className="py-6 space-y-4 animate-in fade-in zoom-in-95 duration-300 overflow-hidden">
            {isBatch ? (
                <div className="space-y-4 text-center">
                    ...
                    <p className="text-sm font-medium text-foreground">
                        Invitations successfully sent to:
                    </p>
                    <div className="bg-accent/30 rounded-xl max-h-32 overflow-y-auto p-2 text-left border border-border/40">
                        {invitedEmails.map((email, idx) => (
                            <div key={idx} className="text-[12px] text-muted-foreground flex items-center gap-2 py-1 px-2">
                                <Check className="w-3 h-3 text-primary shrink-0" /> <span className="truncate">{email}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <p className="text-[11px] text-muted-foreground italic text-center px-4">
                {isBatch ? "They will receive an email shortly with instructions to join." : "The recipient can use this link to join the team directly."}
            </p>
