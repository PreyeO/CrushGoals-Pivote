...
export function StatusCard({ title, value, icon: Icon, trend, color, bg, inverse }: StatusCardProps) {
    const trendColor = inverse ? "bg-rose-500/10 text-rose-500" : (color.includes('text-') ? color.replace('text-', 'bg-').replace(' ', '/10 ') + "/10 " + color : color);
    // Actually simpler:
    ...
    return (
        <Card className="glass-card shadow-sm hover:shadow-md transition-shadow duration-300 border-border/40 overflow-hidden relative group">
            ...
            <CardContent className="relative z-10">
                ...
                {trend && (
                    <p className={`text-[11px] mt-2 font-bold px-1.5 py-0.5 rounded-md inline-block ${trendBg} ${finalTrendColor}`}>
                        {trend}
                    </p>
                )}
            </CardContent>
